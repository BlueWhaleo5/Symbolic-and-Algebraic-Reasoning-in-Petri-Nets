...
def bdd_reachable(pn: PetriNet) -> Tuple[BinaryDecisionDiagram, int]:
    """Tính tập reachable markings sử dụng BDD"""
    print("🚀 SYMBOLIC BDD REACHABILITY - 1-BOUNDED PETRI NETS")
    print(f"Places: {pn.place_ids}")
    print(f"Initial: {pn.M0}")
    
    # Tạo biến BDD cho các places
    bdd_vars = {p_name: bddvar(p_name) for p_name in pn.place_ids}
    
    # Kiểm tra conservative net
    is_conservative = _check_conservative(pn)
    print(f"Conservative: {is_conservative}")
    
    # Khởi tạo tập reachable với marking ban đầu
    R = _marking_to_bdd(pn.M0, pn.place_ids, bdd_vars)
    
    # Thêm constraint nếu là conservative net
    if is_conservative:
    ...
    while R_old is None or not R.equivalent(R_old):
        ...
        # Tính image qua tất cả transitions
        ...
        # Áp dụng constraint cho conservative nets
        if is_conservative:
            R_next = R_next & constraint
            
        R = R_next
        
        if iteration >= 20:  # Giới hạn iteration
            break
...
def _check_conservative(pn):
    """Kiểm tra xem Petri net có conservative không"""
    n_transitions = min(len(pn.trans_ids), pn.I.shape[0])
...
def _compute_transition_image(R, trans_idx, pn, bdd_vars, is_conservative):
    """Tính image của tập marking qua một transition"""
    places = pn.place_ids
    n_places = len(places)
    
    # Điều kiện kích hoạt transition
    ...
    for j in range(n_places):
        ...
        if is_input and not is_output:
            # Input only: place phải có token
            enabling &= bdd_vars[places[j]]
        elif is_output and not is_input:
            # Output only: place phải không có token (1-bounded)
            enabling &= ~bdd_vars[places[j]]
        elif is_input and is_output:
            # Self-loop: place phải có token
            enabling &= bdd_vars[places[j]]
    ...
    # Tính các states mới sau khi fire transition
    ...
    for assignment in _get_bdd_assignments(enabled_states, bdd_vars, places):
        ...
        for j in range(n_places):
            ...
            if is_input and not is_output:
                new_marking[places[j]] = 0  # Remove token
            elif is_output and not is_input:
                new_marking[places[j]] = 1  # Add token
            # Self-loop: giữ nguyên
        
        # Kiểm tra conservative constraint
        if is_conservative:
...
def _create_token_constraint(total_tokens, place_ids, bdd_vars):
    """Tạo constraint tổng số tokens không đổi"""
    from itertools import combinations
    
    constraint = _bdd_false(bdd_vars)
    n_places = len(place_ids)
    
    # Tạo tất cả combinations có đúng số tokens
...
def _get_bdd_assignments(bdd, bdd_vars, places):
    """Lấy tất cả assignments thỏa mãn BDD"""
    if bdd.is_zero():
        return []
...
def _bdd_satisfy_count(bdd, bdd_vars, places):
    """Đếm số assignments thỏa mãn BDD"""
