...
def optimi_reachable_marking(
    ...
    """
    Find the reachable marking that maximizes the linear objective function c^T * M.
    Brute force approach that checks all possible markings.
    """
    if bdd.is_zero():
        return None, None
    
    if bdd.is_one():
        # All markings are reachable - choose the one that maximizes objective
    ...
    # Try all possible 0-1 markings (2^n possibilities)
    for i in range(2**n):
        marking = []
        for j in range(n):
            marking.append(1 if (i >> j) & 1 else 0)
        
        # Create assignment for BDD checking
        assignment = {}
        for var, idx in var_mapping.items():
            assignment[var] = bool(marking[idx])
        
        # Check if this marking is reachable (satisfies the BDD)
        if bdd.restrict(assignment).is_one():
            ...
            if value > best_value:
                ...
                # Tie-breaking: use lexicographical order
